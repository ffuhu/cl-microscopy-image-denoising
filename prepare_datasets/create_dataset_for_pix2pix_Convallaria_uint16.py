...
path = root_folder + '/Convallaria_diaphragm/'
zip_file = 'Convallaria_diaphragm.zip'
url_download_from = 'https://cloud.mpi-cbg.de/index.php/s/BE8raMtHQlgLDF3/download'
...
image_stack_annotation = np.repeat(image_stack_annotation, 100, axis=0)
...
image_stack_images_train = np.concatenate((image_stack_images[:, 512:, 512:],
                                           image_stack_images[:, :512, 512:],
                                           image_stack_images[:, 512:, :512]))
image_stack_annotation_train = np.concatenate((image_stack_annotation[:, 512:, 512:],
                                               image_stack_annotation[:, :512, 512:],
                                               image_stack_annotation[:, 512:, :512]))

image_stack_images_test = image_stack_images[:, :512, :512]
image_stack_annotation_test = image_stack_annotation[:, :512, :512]
